...
# This are the coordinates of a bounding box used for image processing
x1, y1, x2, y2 = 0, 45, 550, 115
...
while True:
    ...
    for keypoint in keypoints:
        ...
        cv2.circle(img, (int(keypoint.pt[0]), int(keypoint.pt[1]) + y1), int(keypoint.size / 2), (0, 0, 255), -1)

        # Draw center of blob with BLACK colour
        cv2.circle(img, (int(keypoint.pt[0]), int(keypoint.pt[1]) + y1), 3, (0, 0, 0), -1)

        # Put Text near each detected Blob
        cv2.putText(img, "Blob", (int(keypoint.pt[0]) - 25, int(keypoint.pt[1]) + 55 + y1),
    ...
    # Digital Image Processing Boundary box
    cv2.line(img, (x1, y1), (x1, y2), (255, 255, 255), 1)  # DIP border 1
    cv2.line(img, (x2, y1), (x2, y2), (255, 255, 255), 1)  # DIP border 2
    cv2.line(img, (x1, y1), (x2, y1), (255, 255, 255), 1)  # DIP border 2
    cv2.line(img, (x1, y2), (x2, y2), (255, 255, 255), 1)  # DIP border 2
...
print("Total time: ", end_time - start_time)  # Print the total time taken to run the code
